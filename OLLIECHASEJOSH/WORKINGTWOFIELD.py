"""
Two-field LBM ink-on-paper — ring-biased version
- rho      : solvent density (LBM, D2Q9)
- phi      : mobile pigment concentration
- deposit  : immobile deposited pigment

Changes from previous version:
- stronger ring bias: deposition depends mainly on evaporation, not just thinness
- lower pigment diffusion
- gentle pinning re-enabled
- saves every figure to disk
"""
...
for step in range(steps):
    ...
    # updated solvent fields
    rho = f.sum(dim=0)
    momentum = torch.einsum('ic,ixy->cxy', c, f)
    u = momentum / (rho.unsqueeze(0) + 1e-10)

    # pigment
    phi, deposit = advect_diffuse_phi(phi, deposit, rho, u, J, kappa, pin_active)
...
dep_cmap = LinearSegmentedColormap.from_list('dep', [
    (0.97, 0.98, 1.00),   # near-white, faint blue tint
    (0.70, 0.80, 0.92),   # pale blue
    (0.25, 0.45, 0.75),   # mid blue
    (0.04, 0.08, 0.25)    # deep navy
])
