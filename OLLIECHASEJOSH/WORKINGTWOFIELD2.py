...
def advect_diffuse_phi(phi, deposit, rho, u, J, kappa, pin_active):
    ...
    # lower mobility in dense fiber regi

    m_adv = advect_upwind(m, ux, uy)
    m_adv = m_adv + 0.002 * laplacian(m)
    m_adv = torch.clamp(m_adv, min=0.0)

    # deposition: keep this as the thin-film/rim mechanism
    thin_factor = torch.clamp((thin_film_thresh - h) / thin_film_thresh, 0.0, 1.0)

    # optional: use evaporation to strengthen rim deposition
    J_norm = J / (J.max() + 1e-10)
    dep_prob = 0.01 * (0.7 * thin_factor**2 + 0.3 * J_norm) * wet_f
    dep_prob = torch.clamp(dep_prob, 0.0, 0.08)

    dm_dep = dep_prob * m_adv
